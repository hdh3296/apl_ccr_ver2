...
far uint8_t * source_ptr = (far uint8_t *)new_value;                    /*pointers to data*/
far uint8_t * dest_ptr = (far uint8_t *)information;
uint16_t size = (FLASH_ONE_BLOCK_SIZE);						
...
uint16_t FlashBlockRd(uint16_t BlkNm)
{
	...
	if(BlkNm <= MAX_FLASH_BLOCK_NM){	
		j= (BlkNm * FLASH_ONE_BLOCK_SIZE);		
	    for(i=0;i<(size/FLASH_ONE_RECORD_BYTE);i++){
	        new_value[i].long_data=information[j+i].long_data;
	    }
		return(1);		
	}
	return(0);
...
uint16_t  FlashBlockWr(uint16_t BlkNm) 
{
	...
	if(BlkNm <= MAX_FLASH_BLOCK_NM){	
		j= (BlkNm * FLASH_ONE_BLOCK_SIZE);
    	flash_write(source_ptr,size,(dest_ptr+j));
		FlashBlockRd(BlkNm);
		return(1);
	}
	return(0);
